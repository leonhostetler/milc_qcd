...
#include "../generic_ks/generic_ks_includes.h"	/* definitions files and prototypes */
...
load_X_from_W(info_t *info, fn_links_t *fn, hisq_auxiliary_t *aux,
  ...
  // Replace X links with U links in the fat links
  int i;
  ...
  load_lnglinks(info, lng, ap, aux->U_link ); // Replace W links with U links in the long links
  final_flop += info->final_flop;
#endif
  dtime += dclock();
